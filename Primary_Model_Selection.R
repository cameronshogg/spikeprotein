...
{
  validSet <- train[validSetSplits==k,]
  trainSet <- train[validSetSplits!=k,]
  ...
  RMSE1_t[k] <- (mean(m_k_bic$residuals^2)) #Rmse on train
  ...
  RMSE2_t[k] <- (mean(m_k_bic_2$residuals^2)) #Rmse on train
  ...
  RMSE3_t[k] <- (mean(m_k_bic_3$residuals^2)) #Rmse on train
  
  #Model 4 - BIC with half penalty
  
  m_k_bic_4 <- stepAIC(object = empty, scope = list(upper = full, lower = empty),
                       direction = "both", trace = 0, k = 1/2 * log(nrow(trainSet)))
  pred4 <- predict(m_k_bic_4, newdata = validSet)
  RMSE4[k] <- sqrt(mean((validSet$accuracy - pred4)^2))
  RMSE4_t[k] <- (mean(m_k_bic_4$residuals^2)) #Rmse on train
  ...
  RMSE5[k] = sqrt(mean((validSet$accuracy - predICM)^2)) # RMSE on validation
  RMSE5_t[k] = sqrt(mean(best.model$residuals^2)) # RMSE on train
  ...
  RMSE6_t[k] <- (mean(m6$residuals^2)) #Rmse on train
  
  #Model 7 - ICM with half BIC penalty
  
  pen <- log((1/2)*nrow(trainSet))
  varlist = c()
  varnames = names(trainSet)
  n = nrow(trainSet)
  varorder <- sample(1:ncol(trainSet))
  ...
  predICM <- predict(best.model, newdata = validSet)
  RMSE7[k] = sqrt(mean((validSet$accuracy - predICM)^2)) # RMSE on validation
  RMSE7_t[k] = sqrt(mean(best.model$residuals^2)) # RMSE on train
  ...
  pred1 <- predict(m_k_bic, newdata = validSet)
  RMSE8[k] <- sqrt(mean((validSet$accuracy - pred1)^2))
  RMSE8_t[k] <- (mean(m_k_bic$residuals^2)) #Rmse on train
  
  #Model 9 - ICM with half BIC penalty
  ...
  predICM <- predict(best.model, newdata = validSet)
  RMSE9[k] = sqrt(mean((validSet$accuracy - predICM)^2)) # RMSE on validation
  RMSE9_t[k] = sqrt(mean(best.model$residuals^2)) # RMSE on train
